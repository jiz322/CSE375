...
public:
	...
	void run(vector<Point> & points)
	{
		...
		for(int i = 0; i < K; i++)
		{
			int j = 0; //NB replace rand() by fake random to make my measurements actually make sense.
			while(true)
			{
				j = j + 1;
...
int main(int argc, char *argv[])
{
	...
	int partitian = total_points/(700*K) + 1; 

	vector<vector<Point>> vec_points;
	string point_name;

	for (int j = 0; j < partitian; j++)
	{
		vector<Point> points;
		for(int i = j*total_points/partitian; i < (j+1)*total_points/partitian; i++)
		{
			vector<double> values;


			for(int m = j*total_points/partitian; m < (j+1)*total_points/partitian; m++)
			{
				double value;
				cin >> value;
				values.push_back(value);
		...
		KMeans kmeans(K, vec_points[j].size(), total_values, max_iterations);
